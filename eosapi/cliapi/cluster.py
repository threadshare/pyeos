...
from eosapi.cliapi.node import Node
from . import CORE_SYMBOL
from eosapi.cliapi.wallet import WalletMgr
...
class Cluster(object):
    ...
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost",
                 walletPort=8899, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="EOStest",
    ...
    def initializeNodes(self, defproduceraPrvtKey=None, defproducerbPrvtKey=None, onlyBios=False):
        ...
        node = Node(host, port, enableMongo=self.enableMongo, mongoHost=self.mongoHost, mongoPort=self.mongoPort,
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                ...
                if Utils.Debug: Utils.Print("name: %s, key(owner): ['%s', '%s], key(active): ['%s', '%s']" % (
    ...
    def bootstrap(totalNodes, prodCount, biosHost, biosPort, dontKill=False, onlyBios=False):
        ...
        try:
            ...
            abiFile = "contracts/%s/%s.abi" % (contract, contract)
            Utils.Print("Publish %s contract" % (contract))
            trans = biosNode.publishContract(eosioTokenAccount.name, contractDir, wastFile, abiFile,
    ...
    def discoverLocalNodes(self, totalNodes, timeout=0):
        ...
        for i in range(0, totalNodes):
            ...
            if m is None:
                ...
                break
            instance = Node(self.host, self.port + i, pid=int(m.group(1)), cmd=m.group(2), enableMongo=self.enableMongo,
